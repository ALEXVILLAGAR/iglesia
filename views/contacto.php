<div class="row bajar ocultar" id="mi-contacto">

	<h2 class="col-md-12 text-center sombra ">Contacto.</h2>
	<br>
	<div class="row text-center">
		<div class="card text-white bg-secondary mb-3 sombra col-md-3 ml-10" style="max-width: 18rem;">
		  <div class="card-header bg-primary2">
		  	<strong class="text-center ml-4 mt-5"> Reunión Abuelos Felices</strong></div>
		  <div class="card-body p-2">
		  	<br>
		    <h5 class="card-title text-center">Último viernes de cada mes</h5>
		    <br>
		    <p class="card-text text-center">
		                desde las 3:00 pm
		    </p>
		  </div>
		</div>

		<div class="card text-white bg-secondary mb-3 sombra col-md-3  ml-5" style="max-width: 18rem;">
		  <div class="card-header bg-primary2">
		  	<strong class="text-center ml-4 mt-5"> Reuniones generales</strong></div>
		  <div class="card-body p-2">
		  	<br>
		    <h5 class="card-title text-center">Los domingos tenemos Escuela dominical para niños</h5>
		    <p class="card-text text-center">
		                8:00 am y 10:30 am
		    </p>
		    <hr class="bg-green ">
		    <h5 class="card-title text-center">Los sábados tenemos Reunión Juvenil “Fire Soul”</h5>
		    <br>
		    <p class="card-text text-center">
		                7:00 pm
		    </p>
		  </div>
		</div>

		<div class="card text-white bg-secondary mb-3 sombra col-md-3 ml-5" style="max-width: 18rem;">
		  <div class="card-header bg-primary2">
		  	<strong class="text-center ml-4 mt-5"> Horarios de oficina
             </strong>
         </div>
		  <div class="card-body p-2">
		  	<br>
		    <h5 class="card-title text-center"> Lunes a viernes: </h5>
		    <br>
		    <p class="card-text text-center">
				Mañana: 8:00 am a 12:00 pm
				Tarde: 2:00 pm a 6:00 pm
				<hr class="bg-green">
				<h5 class="card-title text-center">Sábados: </h5>
				<br>
				9:00 am a 1:00 pm
		    </p>
		  </div>
		</div>
         
		<div class="card col-md-10 bg-secondary text-white ml-10 sombra">
  <div class="card-header bg-primary2">
    <h4><strong>Domicilio</strong></h4>
  </div>
  <div class="card-body m-4">
    <h5 class="card-title"><strong>Acércate a nuestra iglesia</strong></h5>
    <p class="card-text">CRA 16 N° 18-40, Santa Rosa de Cabal, Risaralda.
    <br>
         Teléfono: 57 (6) 3655317
    </p>
   </div>
   <div> 
   	<h4>nuestras redes</h4>
   	<hr class="col-md-1">
   	<a href="https://www.youtube.com/channel/UCHArXYSb6l9-J6sCYOjDHAQ?view_as=subscriber" title="" target="blank"><i class="fab fa-youtube fa-3x text-danger"></i></a>
   <a href="https://www.facebook.com/IICCMD/" title="" target="blank" class="ml-2"><i class="fab fa-facebook-square fa-3x text-info"></i></a>
   <br>
</div>
  
</div>
    </div>
</div>