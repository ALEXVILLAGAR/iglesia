
<h4 class="col-md-12 text-center mt-5">Ministerios</h4>
<hr>

<h5 class="col-md-12 text-center mt-5">Alabanza</h5>
<hr class="col-md-1">

<div class="row mt-4">
  <?php 
   $casas = 'cuantas_fotos';
   $casas('ministerios/alabanza',3);  // Esto llama a cuantas_fotos
?>

</div>
<div class="row">
  <button class="button3 mt-2 col-md-8 offset-2" id="alabanza"><strong><i class="fas fa-images fa-2x"></i> Ver todas las fotos</strong>
    <?php $archivo='fotos2' ?></button>
</div>

<h5 class="col-md-12 text-center mt-5">Infantil</h5>
<hr class="col-md-1">

<div class="row mt-4">
  <?php 
   $casas = 'cuantas_fotos';
   $casas('ministerios/infantil',3);  // Esto llama a cuantas_fotos
?>

</div>

<div class="row">
   <button class="button3 mt-2 col-md-8 offset-2" id="infantil"><strong><i class="fas fa-images fa-2x"></i> Ver todas las fotos</strong>
    <?php $archivo='fotos2' ?></button>
</div>
 


<h5 class="col-md-12 text-center mt-5">Ujieres</h5>
<hr class="col-md-1">

<div class="row mt-4">
  <?php 
   $casas = 'cuantas_fotos';
   $casas('ministerios/Ujieres',3);  // Esto llama a cuantas_fotos
?>

</div>

<div class="row">
    <button class="button3 mt-2 col-md-8 offset-2" id="ujieres"><strong><i class="fas fa-images fa-2x"></i> Ver todas las fotos</strong>
    <?php $archivo='fotos2' ?></button>
</div>
